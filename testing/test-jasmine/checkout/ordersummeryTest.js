...
import { loadingFormsStorage } from '../../../data/cart.js';
import { loadFetch } from '../../../data/products.js';

// integration testing is the combination of unit testing and end to end testing
describe('test suite: testing the renderPage function form orderSummery.js', ()=> {
  ...
  beforeAll((done)=>{
   //loadProduct(done); //--> This loadProduct function is for getting the product data from the backend server and that done() function we give argument to the loadProduct() and we run the done() function after getting the data so that done() run the full test after getting the data from server.
    async function fetchProducts(){
        await loadFetch()
        done(); //--> This done() function just tell the testing framework that the async operation is complete and now you can run the test cases.
    }
    fetchProducts();
  });
  ...
  it('testing the delete cart link', ()=>{
    ...
    expect( localStorage.setItem).toHaveBeenCalledTimes(2); //--> This just check how many times the localStorage.setItem called after deleting one product from the cart.
  });
